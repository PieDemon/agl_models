...
def calculate_smoothed_win_rate(wins, matches, prior_weight=2):
    """Returned to the original honest 2-game prior baseline"""
    if matches == 0:
        return 0.50
...
def generate_power_leaderboard():
    ...
    # 🔍 PASTE YOUR NEW RAW COEFFICIENTS FROM STEP 1 HERE:
    # (Below are placeholder approximations, replace them with your terminal output)
    COEFFICIENTS = {
        "alltime_winrate_diff": 13.361626,  
        "league_winrate_diff": -1.323635,  
        "alltime_experience_diff": 0.000546,  # Raw experience coefficients will be very small now
        "league_experience_diff": 0.068337,   
        "pre_match_elo_diff": -0.011878       # Raw Elo coefficients will be very small now
    }
    INTERCEPT = 0.001624 
    ...
    for idx, p in df_pop.iterrows():
        # 🔍 FIX: Evaluating raw delta differences against the population average.
        # No standard deviation division!
        diff_alltime_wr = p["alltime_winrate"] - means["alltime_winrate"]
        diff_league_wr = p["league_winrate"] - means["league_winrate"]
        diff_alltime_exp = p["alltime_experience"] - means["alltime_experience"]
        diff_league_exp = p["league_experience"] - means["league_experience"]
        diff_elo = p["pre_match_elo"] - means["pre_match_elo"]

        # Linear combination using honest raw math
        log_odds = (
            INTERCEPT +
        ...
        # Pure, un-squashed standard sigmoid function
