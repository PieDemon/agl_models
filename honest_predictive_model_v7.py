...
def train_raw_mtg_predictor():
    ...
    for match in matches:
        ...
        # 🔍 FIX: Feeding RAW unscaled delta differences
        features.append({
    ...
    # 🔍 FIX: Train directly on raw unscaled numbers. 
    # C=0.1 applies a gentle brake to handle multicollinearity without standard scaling.
    model = LogisticRegression(penalty='l2', C=0.1, max_iter=2000)
    model.fit(X_train, y_train)

    accuracy = accuracy_score(y_test, model.predict(X_test))
    
    print("\n================ 🛡️ RAW UNSCALED MODEL RESULTS ================")
    print(f"Honest Match Prediction Accuracy: {accuracy * 100:.2f}%")
    print("------------------------------------------------------------------")
    print("Raw Coefficients (Copy these exact numbers into your leaderboard script!):")
    
    for col, coef in zip(df.columns, model.coef_.ravel()):
        print(f"  {col.ljust(25)} : {coef:+.6f}")
    print(f"  intercept                 : {model.intercept_[0]:.6f}")
    print("==================================================================\n")

if __name__ == "__main__":
    train_raw_mtg_predictor()
