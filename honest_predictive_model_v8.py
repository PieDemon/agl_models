...
def train_perfect_scaled_predictor():
    ...
    # Standardize features using the production scaler
    ...
    print("\n================ 🛡️ MODEL CALIBRATION COMPLETED ================")
    print(f"Verified Accuracy: {accuracy * 100:.2f}%")
    print("------------------------------------------------------------------")
    print("1. STANDARDIZED COEFFICIENTS:")
    for col, coef in zip(df.columns, model.coef_.ravel()):
        print(f"   {col.ljust(25)} : {coef:+.6f}")
    print(f"   intercept                 : {model.intercept_[0]:.6f}")
    
    print("\n2. SCALER SCALE FACTORS (Copy these exact numbers into the leaderboard script!):")
    for col, scale in zip(df.columns, scaler.scale_):
        print(f"   {col.ljust(25)} : {scale:.6f}")
    print("==================================================================\n")

if __name__ == "__main__":
    train_perfect_scaled_predictor()
