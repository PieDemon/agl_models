...
def calculate_smoothed_win_rate(wins, matches, prior_weight=10):
    """
    🔍 RE-TUNED: Increased prior_weight to 10 to heavily penalize 
    low-sample players (like 2-0 or 5-0) from dominating veterans.
    """
    if matches == 0:
        return 0.50
...
def generate_power_leaderboard():
    ...
    # 3. Score players with a mathematical squashing function
    for idx, p in df_pop.iterrows():
        ...
        # Standard raw sigmoid probability
        raw_prob = 1 / (1 + np.exp(-log_odds))
        
        # 🔍 CRITICAL FIX: Squash extreme values to force realistic card game thresholds
        # This prevents low-sample players from skyrocketing to 99%
        win_probability = 0.35 + (raw_prob * 0.40) # Compresses range to exactly 35% - 75%
        
        # Scale Power Rating naturally around 1000 baseline
        power_rating = round(1000 + (win_probability - 0.5) * 1500)
