...
def calculate_bayesian_shrunk_rate(wins, matches, global_mean=0.50, sample_anchor=25):
    """
    🔍 THE ADVANCED FIX: Uses a dynamic Bayesian Shrinkage anchor.
    If a player has fewer than 25 matches, their win rate is heavily 
    pulled back toward the 50% league average to eliminate small-sample bias.
    """
...
def generate_power_leaderboard():
    ...
    # 2. Compute Bayesian Shrunk rates for the population
    players_list = []
    for name, stats in player_latest_states.items():
        players_list.append({
            "name": name,
            # Using the 25-match Bayesian anchor to steady low-volume spikes
    ...
    # 3. Model parameters from your 68.08% scaled configuration
    COEFFICIENTS = {
    ...
    # Scale normalization values from your model configuration
    SCALE_FACTORS = {
    ...
    print("Simulating Bayesian-regularized round-robin gauntlet...")
